...
- **Três marcos de tempo, e cada sinal do CLP manda em um só.** `DataInicio` é o clique do operador. `REGISTRO_RODANDO` inicia e para o registro: a borda de descida dele — sozinha — fecha a etapa e define `DataFim`. `INICIA_CONTAGEM` manda só na contagem de tempo: as bordas dele carimbam `DataInicioContagem`/`DataFimContagem`, que é a janela que o laudo analisa e o tempo que a tela mostra. Falha de leitura do `INICIA_CONTAGEM` nunca trava o encerramento — degrada para a regra antiga do setpoint.
